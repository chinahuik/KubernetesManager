 ...
 */
@RestController
@RequestMapping("/KubernetesManager/base-image")
public class BaseImageController {
	@Autowired
	private BaseImageService service;

	protected static final Logger logger = LoggerFactory
			.getLogger(BaseImageController.class);

	@ApiOperation(value = "增加BaseImage", notes = "表的REST接口")
	@ApiImplicitParam(name = "record", value = "", required = true,
			dataType = "BaseImage", dataTypeClass = BaseImage.class, paramType = "body")
	@RequestMapping(value = "/create.do", method = RequestMethod.POST)
	@ResponseBody
	public BaseResponse create(@ApiParam @RequestBody BaseImage record) {
		try {
			service.save(record);
	...
	@ApiOperation(value = "删除BaseImage", notes = "的REST接口")
	@ApiImplicitParam(name = "id", value = "BaseImage编号", required = true,
	...
	@ApiOperation(value = "获取BaseImage详情", notes = "的REST接口")
	@ApiImplicitParam(name = "id", value = "BaseImage", required = true,
			dataType = "Integer", paramType = "query")
	@RequestMapping(value = "/getById.do", method = RequestMethod.GET)
	@ResponseBody
	public BaseResponse findOne(@ApiParam @RequestParam(value = "id") Integer id) {
		try {
			final BaseImage record = service.getById(id);
			return new ObjectResponse<BaseImage>(record);
		} catch (final Exception exception) {
			logger.error("find BaseImage failed {}", exception);
			return new BaseResponse("10003", exception.getMessage());
		}
	}

	@ApiOperation(value = "获取BaseImage列表", notes = "表的REST接口")
	@ApiImplicitParams({
	...
	public BaseResponse list(
		...
		try {
			...
			final List<BaseImage> list = service
					.page(new Page<BaseImage>(currentPage, pageSize), entity)
					.getRecords();
			final int total = service.count(entity);
			final BaseResponse response = new ListResponse<BaseImage>(list);
			response.put("total", total);
			return response;
		} catch (final Exception exception) {
			logger.error("list BaseImage failed {}", exception);
			return new BaseResponse("10001", exception.getMessage());
		}
	}

	@ApiOperation(value = "更新BaseImage", notes = "的REST接口")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "BaseImage编号", required = true,
					dataType = "Integer", paramType = "query"),
			@ApiImplicitParam(name = "record", value = "BaseImage", required = true,
					dataType = "BaseImage", dataTypeClass = BaseImage.class) })
	@RequestMapping(value = "/update.do", method = RequestMethod.POST)
	@ResponseBody
	public BaseResponse save(@ApiParam @RequestParam(value = "id") Integer id,
			@ApiParam @ModelAttribute BaseImage record) {
		try {
