 ...
 */
@RestController
@RequestMapping("/KubernetesManager/kube-node")
public class KubeNodeController {
	@Autowired
	private KubeNodeService service;

	protected static final Logger logger = LoggerFactory
			.getLogger(KubeNodeController.class);

	@ApiOperation(value = "增加KubeNode", notes = "表的REST接口")
	@ApiImplicitParam(name = "record", value = "", required = true, dataType = "KubeNode",
			dataTypeClass = KubeNode.class, paramType = "body")
	@RequestMapping(value = "/create.do", method = RequestMethod.POST)
	@ResponseBody
	public BaseResponse create(@ApiParam @RequestBody KubeNode record) {
		try {
			service.save(record);
	...
	@ApiOperation(value = "删除KubeNode", notes = "的REST接口")
	@ApiImplicitParam(name = "id", value = "KubeNode编号", required = true,
	...
	@ApiOperation(value = "获取KubeNode详情", notes = "的REST接口")
	@ApiImplicitParam(name = "id", value = "KubeNode", required = true,
			dataType = "Integer", paramType = "query")
	@RequestMapping(value = "/getById.do", method = RequestMethod.GET)
	@ResponseBody
	public BaseResponse findOne(@ApiParam @RequestParam(value = "id") Integer id) {
		try {
			final KubeNode record = service.getById(id);
			return new ObjectResponse<KubeNode>(record);
		} catch (final Exception exception) {
			logger.error("find KubeNode failed {}", exception);
			return new BaseResponse("10003", exception.getMessage());
		}
	}

	@ApiOperation(value = "获取KubeNode列表", notes = "表的REST接口")
	@ApiImplicitParams({
	...
	public BaseResponse list(
		...
		try {
			...
			final List<KubeNode> list = service
					.page(new Page<KubeNode>(currentPage, pageSize), entity).getRecords();
			final int total = service.count(entity);
			final BaseResponse response = new ListResponse<KubeNode>(list);
			response.put("total", total);
			return response;
		} catch (final Exception exception) {
			logger.error("list KubeNode failed {}", exception);
			return new BaseResponse("10001", exception.getMessage());
		}
	}

	@ApiOperation(value = "更新KubeNode", notes = "的REST接口")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "KubeNode编号", required = true,
					dataType = "Integer", paramType = "query"),
			@ApiImplicitParam(name = "record", value = "KubeNode", required = true,
					dataType = "KubeNode", dataTypeClass = KubeNode.class) })
	@RequestMapping(value = "/update.do", method = RequestMethod.POST)
	@ResponseBody
	public BaseResponse save(@ApiParam @RequestParam(value = "id") Integer id,
			@ApiParam @ModelAttribute KubeNode record) {
		try {
