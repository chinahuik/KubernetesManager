 ...
 */
@RestController
@RequestMapping("/KubernetesManager/kube-service")
public class KubeServiceController {
	@Autowired
	private KubeServiceService service;

	protected static final Logger logger = LoggerFactory
			.getLogger(KubeServiceController.class);

	@ApiOperation(value = "增加KubeService", notes = "表的REST接口")
	@ApiImplicitParam(name = "record", value = "", required = true,
			dataType = "KubeService", dataTypeClass = KubeService.class,
			paramType = "body")
	@RequestMapping(value = "/create.do", method = RequestMethod.POST)
	@ResponseBody
	public BaseResponse create(@ApiParam @RequestBody KubeService record) {
		try {
			service.save(record);
	...
	@ApiOperation(value = "删除KubeService", notes = "的REST接口")
	@ApiImplicitParam(name = "id", value = "KubeService编号", required = true,
	...
	@ApiOperation(value = "获取KubeService详情", notes = "的REST接口")
	@ApiImplicitParam(name = "id", value = "KubeService", required = true,
			dataType = "Integer", paramType = "query")
	@RequestMapping(value = "/getById.do", method = RequestMethod.GET)
	@ResponseBody
	public BaseResponse findOne(@ApiParam @RequestParam(value = "id") Integer id) {
		try {
			final KubeService record = service.getById(id);
			return new ObjectResponse<KubeService>(record);
		} catch (final Exception exception) {
			logger.error("find KubeService failed {}", exception);
			return new BaseResponse("10003", exception.getMessage());
		}
	}

	@ApiOperation(value = "获取KubeService列表", notes = "表的REST接口")
	@ApiImplicitParams({
	...
	public BaseResponse list(
		...
		try {
			...
			final List<KubeService> list = service
					.page(new Page<KubeService>(currentPage, pageSize), entity)
					.getRecords();
			final int total = service.count(entity);
			final BaseResponse response = new ListResponse<KubeService>(list);
			response.put("total", total);
			return response;
		} catch (final Exception exception) {
			logger.error("list KubeService failed {}", exception);
			return new BaseResponse("10001", exception.getMessage());
		}
	}

	@ApiOperation(value = "更新KubeService", notes = "的REST接口")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "KubeService编号", required = true,
					dataType = "Integer", paramType = "query"),
			@ApiImplicitParam(name = "record", value = "KubeService", required = true,
					dataType = "KubeService", dataTypeClass = KubeService.class) })
	@RequestMapping(value = "/update.do", method = RequestMethod.POST)
	@ResponseBody
	public BaseResponse save(@ApiParam @RequestParam(value = "id") Integer id,
			@ApiParam @ModelAttribute KubeService record) {
		try {
