...
@EnableAsync // 2.开启多线程
@Slf4j
public class KubeSyncDaemonsetThread {
	@Autowired
	...
	@Value("${job.sync.daemonset.enabed}")
	private boolean cronEnabled;

	@Async("taskExecutor")
	@Scheduled(cron = "${job.sync.daemonset.cron}")
	// @Scheduled(fixedDelay = 1000) // 间隔1秒
	public void execute() {
		if (!cronEnabled) {
			log.info("kube sync daemonset job not enabled.");
			return;
		}
					...
					for (final KubeNamespace namespace : namespaces) {
						...
						if (resp != null && !resp.trim().isEmpty()) {
							...
							if (object.getString("kind")
									.equalsIgnoreCase("DaemonSetList")) {
