...
@EnableAsync // 2.开启多线程
@Slf4j
public class KubeSyncDeploymentThread {
	@Autowired
	...
	@Autowired
	private KubeDeploymentService deploymentService;

	@Value("${job.sync.deploy.enabed}")
	private boolean cronEnabled;

	@Async("taskExecutor")
	@Scheduled(cron = "${job.sync.deploy.cron}")
				...
				try (StringReader reader = new StringReader(kubeCluster.getConfig());) {
					...
					for (final KubeNamespace namespace : namespaces) {
						...
						if (resp != null && !resp.trim().isEmpty()) {
							...
							if (object.getString("kind")
									.equalsIgnoreCase("DeploymentList")) {
								final JSONArray deps = object.getJSONArray("items");
								deploymentService.upserts(deps, kubeCluster);
							}
