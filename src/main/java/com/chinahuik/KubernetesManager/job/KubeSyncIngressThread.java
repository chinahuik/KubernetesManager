...
@EnableAsync // 2.开启多线程
@Slf4j
public class KubeSyncIngressThread {
	@Autowired
	...
	@Value("${job.sync.ingress.enabed}")
	private boolean cronEnabled;

	@Async("taskExecutor")
	@Scheduled(cron = "${job.sync.ingress.cron}")
	// @Scheduled(fixedDelay = 1000) // 间隔1秒
	public void execute() {
		if (!cronEnabled) {
			log.info("kube sync ingress job not enabled.");
			return;
		}
		...
		if (clusters != null) {
			for (final KubeCluster kubeCluster : clusters) {
				try (StringReader reader = new StringReader(kubeCluster.getConfig());) {
					final Wrapper<KubeNamespace> entity     = new QueryWrapper<KubeNamespace>().eq("cluster_id",
							kubeCluster.getId());
					final List<KubeNamespace>    namespaces = namespaceDao.selectList(entity);
					if (namespaces == null) {
						continue;
					}
					...
					for (final KubeNamespace namespace : namespaces) {
						...
						final String api  = String.format("/apis/extensions/v1beta1/namespaces/%s/ingresses",
								namespace.getNamespace().trim());
						final String resp = client.doGet(api);
						log.info("response: {}", resp);
						if (resp != null && !resp.trim().isEmpty()) {
							final JSONObject object = JSONObject.parseObject(resp);
							if (object.getString("kind").equalsIgnoreCase("IngressList")) {
