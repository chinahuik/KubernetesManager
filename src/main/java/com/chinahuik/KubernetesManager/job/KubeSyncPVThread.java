...
@EnableAsync // 2.开启多线程
@Slf4j
public class KubeSyncPVThread {
	@Autowired
	private KubeClusterDao kubeClusterDao;

	@Value("${job.sync.pv.enabed}")
	private boolean cronEnabled;

	@Async("taskExecutor")
	@Scheduled(cron = "${job.sync.pv.cron}")
	// @Scheduled(fixedDelay = 1000) // 间隔1秒
	public void execute() {
		if (!cronEnabled) {
			log.info("kube sync pv job not enabled.");
			return;
		}
		...
		if (clusters != null) {
			for (final KubeCluster kubeCluster : clusters) {
				try (StringReader reader = new StringReader(kubeCluster.getConfig());) {
					...
					final String resp = client.doGet("/api/v1/persistentvolumes");
		...
		log.info("-----kube sync pv job finished.----");
	}
}
