...
public class SslUtil {
	...
	public static KeyStore createKeyStore(byte[] clientCertData,
			byte[] clientKeyData, String clientKeyAlgo,
			String clientKeyPassphrase, String keyStoreFile,
		...
		try (InputStream certInputStream = new ByteArrayInputStream(
				...
				InputStream keyInputStream = new ByteArrayInputStream(
						clientKeyData)) {
			return createKeyStore(certInputStream, keyInputStream,
					clientKeyAlgo,
					clientKeyPassphrase != null
							? clientKeyPassphrase.toCharArray()
							: null,
					keyStoreFile, getKeyStorePassphrase(keyStorePassphrase));
		}
	}

	public static KeyStore createKeyStore(InputStream certInputStream,
			InputStream keyInputStream, String clientKeyAlgo,
			char[] clientKeyPassphrase, String keyStoreFile,
		...
		keyStore.setKeyEntry(alias, privateKey, clientKeyPassphrase,
	...
	public static RSAPrivateCrtKeySpec decodePKCS1(byte[] keyBytes)
		...
		return new RSAPrivateCrtKeySpec(next(parser), next(parser),
				next(parser), next(parser), next(parser), next(parser),
	...
	public static KeyManager[] keyManagers(byte[] certData, byte[] keyData,
			String algo, String passphrase, String keyStoreFile,
			String keyStorePassphrase) throws NoSuchAlgorithmException,
		...
		if (certData != null && keyData != null) {
			KeyStore keyStore = createKeyStore(certData, keyData, algo,
					passphrase, keyStoreFile, keyStorePassphrase);
	...
	private static byte[] decodePem(InputStream keyInputStream)
			throws IOException {
	...
	private static char[] getKeyStorePassphrase(String keyStorePassphrase) {
		if (keyStorePassphrase == null || keyStorePassphrase.length() == 0) {
			return System
					.getProperty("javax.net.ssl.keyStorePassword", "changeit")
					.toCharArray();
		}
	...
	private static void loadDefaultKeyStoreFile(KeyStore keyStore,
		...
		if (keyStorePath != null && keyStorePath.length() > 0) {
			...
			if (loadDefaultStoreFile(keyStore, keyStoreFile,
					keyStorePassphrase)) {
				return;
			}
		}
	...
	private static boolean loadDefaultStoreFile(KeyStore keyStore,
			File fileToLoad, char[] passphrase)
			throws CertificateException, NoSuchAlgorithmException, IOException {
	...
	private static PrivateKey loadKey(InputStream keyInputStream,
			String clientKeyAlgo) throws IOException, NoSuchAlgorithmException,
			...
			while ((pemObject = pemParser.readObject()) != null) {
				if (pemObject instanceof PEMKeyPair) {
					return new JcaPEMKeyConverter()
							.getKeyPair(((PEMKeyPair) pemObject)).getPrivate();
				}
			}
		}
		...
		throw new InvalidKeySpecException(
				"Unknown type of PKCS8 Private Key, tried RSA and ECDSA");
	}
